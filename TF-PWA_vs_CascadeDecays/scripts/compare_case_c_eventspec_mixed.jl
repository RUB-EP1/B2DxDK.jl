"""
Case (c): Event-Specific Parent Masses + Mixed Lineshapes
- Place 1 (Lineshapes): Mixed (nominal pole + event-specific daughter masses)
- Place 2 (Parent Masses): Event-specific sqrt((p1+p2)^2)
"""
...
function build_event_context(sampled_p4)
    ...
    # Place 2: Event-specific parent masses (sqrt(p1+p2)^2)
    m_B_root = mass(P_B)
    m_Dst_int = mass(P_Dst)
...
# Place 1: Mixed lineshapes (nominal pole + event-specific daughter masses)
function bwr_lineshape(ctx, m0, width, l, sign)
    ...
    return sign * MultichannelBreitWigner(m0, [(; gsq, ma = mass(ctx.P_Dst), mb = mass(ctx.pDminus), l, d = 3.0)])
end
...
function x2900_bwr_lineshape(ctx, name, l, param_real)
    ...
    return MultichannelBreitWigner(nominal_mass[name], [(; gsq, ma = mass(ctx.pDminus), mb = mass(ctx.pKplus), l, d = 3.0)])
end
...
function main()
    ...
    output_path === nothing && (output_path = joinpath(suite_dir, "amp_data", charge_tag, "case_c_cd_amp.txt"))
    ...
    println("Case (c): Event-Specific Parent Masses + Mixed Lineshapes")
