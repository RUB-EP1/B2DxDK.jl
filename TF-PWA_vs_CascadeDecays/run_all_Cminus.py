"""
Unified Execution Orchestrator: 4-Case Suite (Cminus Charge Configuration)
===========================================================================
...
and produces publication-quality Dalitz plots and error distribution histograms.
Charge configuration: C- (c = -1.0)
"""
...
def main():
    parser = argparse.ArgumentParser(description=f"Run complete 4-case Dalitz comparison suite ({CHARGE_TAG}).")
    ...
    amp_dir = work_dir / "amp_data" / CHARGE_TAG
    plots_dir = work_dir / "plots" / CHARGE_TAG
    ...
    print(f"Charge configuration:      {CHARGE_TAG}")
    ...
    try:
        ...
        for cand in [
            Path(sys.executable).parent / "envs" / "tf-pwa-env" / "python.exe",
            Path("C:/Users/gamma/miniconda3/envs/tf-pwa-env/python.exe"),
        ]:
            if cand.is_file():
                py_exe = str(cand)
                break
    ...
    if not (args.skip_tfpwa and tfpwa_txt.exists()):
        run_cmd(
            [py_exe, str(scripts_dir / "isolated_tfpwa.py"), "--charge", CHARGE_TAG, "--events", str(events_path), "--output", str(tfpwa_txt)],
            f"Evaluating Conversion-Corrected Isolated TF-PWA ({CHARGE_TAG})",
            cwd=work_dir,
        )
    ...
    if not (args.skip_a and case_a_txt.exists()):
        run_cmd(
            ["julia", "-t", "auto", f"--project={julia_project}", str(scripts_dir / "compare_case_a_pdg_mixed.jl"), f"--charge={CHARGE_TAG}", str(events_path), str(case_a_txt)],
            f"Evaluating Case (a): PDG Parent Masses + Mixed Lineshapes ({CHARGE_TAG})",
            cwd=work_dir,
        )
    ...
    if not (args.skip_b and case_b_txt.exists()):
        run_cmd(
            ["julia", "-t", "auto", f"--project={julia_project}", str(scripts_dir / "compare_case_b_pdg_nominal.jl"), f"--charge={CHARGE_TAG}", str(events_path), str(case_b_txt)],
            f"Evaluating Case (b): PDG Parent Masses + Nominal Lineshapes ({CHARGE_TAG})",
            cwd=work_dir,
        )
    ...
    if not (args.skip_c and case_c_txt.exists()):
        run_cmd(
            ["julia", "-t", "auto", f"--project={julia_project}", str(scripts_dir / "compare_case_c_eventspec_mixed.jl"), f"--charge={CHARGE_TAG}", str(events_path), str(case_c_txt)],
            f"Evaluating Case (c): Event-Specific Parent Masses + Mixed Lineshapes ({CHARGE_TAG})",
            cwd=work_dir,
        )
    ...
    if not (args.skip_d and case_d_txt.exists()):
        run_cmd(
            ["julia", "-t", "auto", f"--project={julia_project}", str(scripts_dir / "compare_case_d_eventspec_nominal.jl"), f"--charge={CHARGE_TAG}", str(events_path), str(case_d_txt)],
            f"Evaluating Case (d): Event-Specific Parent Masses + Nominal Lineshapes ({CHARGE_TAG})",
            cwd=work_dir,
        )
    ...
    run_cmd(
        ...
        f"Generating 4-Case Dalitz and Distribution Comparison Plots ({CHARGE_TAG})",
        cwd=work_dir,
    )

    print("\n" + "=" * 70)
    print(f"ALL 4 EVALUATIONS AND PLOTS FOR {CHARGE_TAG} COMPLETED SUCCESSFULLY!")
