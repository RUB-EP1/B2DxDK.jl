...
function matching_factor(ch::DecayChain)
    ...
    i, j = ij_from_k(k)
    q_Rk_0 = HadronicLineshapes.breakup(ms[4], mR, ms[k])
    q_ij_0 = HadronicLineshapes.breakup(mR, ms[i], ms[j])
    d = HRk.ff.d
    _ff_Rk_0 = HRk.ff(ms²[4], mR^2, ms²[k]) / (q_Rk_0 * d)^orbital_momentum(HRk.ff)
    _ff_ij_0 = Hij.ff(mR^2, ms²[i], ms²[j]) / (q_ij_0 * d)^orbital_momentum(HRk.ff)
