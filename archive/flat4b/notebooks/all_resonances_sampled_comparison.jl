...
function relative_difference_plot(path::String, rows; component::Symbol, nbins::Int = 60)
    ...
    scale_exponent = floor(Int, log10(zlim))
    scale = 10.0^scale_exponent
    scaled_grid = grid ./ scale
    scaled_zlim = zlim / scale
    scale_label = @sprintf("1e%d", scale_exponent)
    title = component === :re ?
        "Mean relative Re difference / $(scale_label)" :
        "Mean relative Im difference / $(scale_label)"
    plt = heatmap(
...
function amp2_comparison_plot(path::String, rows; nbins::Int = 60)
    ...
    zmax = isempty(positive) ? 1.0 : maximum(positive)
...
function run_scan()
    ...
    rel_re_path = joinpath(plots_dir, "$(prefix)_single_resonance_rel_re.png")
    rel_im_path = joinpath(plots_dir, "$(prefix)_single_resonance_rel_im.png")
    amp2_path = joinpath(plots_dir, "$(prefix)_single_resonance_amp2.png")
    ...
    println("Saved plots:       ", rel_re_path)
    println("                   ", rel_im_path)
    println("                   ", amp2_path)
